...
Intro.Language = Intro.Language or {}
Intro.Settings.Map = Intro.Settings.Map or {}
...
if SERVER then
    ...
    if FromWorshop then
        if Intro.Settings.VersionDefault != Intro.Settings.VersionCustom then
            hook.Add("PlayerInitialSpawn", "Intro:PlayerInitialSpawnCheckVersionConfig", function(ply)
                if ply:IsSuperAdmin() then
                    timer.Simple(10, function()
                        ply:IntroChatInfo(Intro.GetLanguage("A new version of the config file is available. Please download it."), 1)
                    end)
                end
            end)
        end

        if Intro.Language.VersionDefault != Intro.Language.VersionCustom then
            hook.Add("PlayerInitialSpawn", "Intro:PlayerInitialSpawnCheckVersionLanguage", function(ply)
                if ply:IsSuperAdmin() then
                    timer.Simple(10, function()
                        ply:IntroChatInfo(Intro.GetLanguage("A new version of the language file is available. Please download it."), 1)
                    end)
                end
            end)
        end
            ...
            http.Fetch( "https://gmod-radio-numerix.mtxserv.com/api/version/"..FileSystem..".txt",
                function( body, len, headers, code )
                    if body != Version then
                        hook.Add("PlayerInitialSpawn", "Intro:PlayerInitialSpawnCheckVersionAddon", function(ply)
                            if ply:IsSuperAdmin() then
                                timer.Simple(10, function()
                                    ply:IntroChatInfo(Intro.GetLanguage("A new version of the addon is available. Please download it."), 1)
                                end)
                            end
                        end)
                    end 
